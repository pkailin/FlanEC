...
def main():
    ...
    processed_file = os.path.join(output_dir, "processed_files.json")
    filtered_stats_file = os.path.join(output_dir, "filtered_stats.json")
    ...
    # STEP 1: Process and save all augmented audio files first
    print("Step 1: Augmenting audio files and saving to disk...")
    ...
    # Clear memory before starting transcription
    gc.collect()
    torch.cuda.empty_cache()
    ...
    # STEP 3: Transcribe each augmented audio file and filter by WER
    print("Step 3: Transcribing augmented audio files and filtering by WER...")
    dataset = []
    filtered_stats = {
        "included": 0,
        "filtered_out": 0,
